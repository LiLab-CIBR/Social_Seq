...
GPU-accelerated video processing toolkit, convenient for multi-camera video reading and writing, which is 9 times faster than OpenCV in certain scenarios. After I open-sourced it, the annual download volume is **70,000 times**. See corresponding [documentation](https://pypi.org/project/ffmpegcv).
...
Camera extrinsic calibration (ball calibration) toolkit. Based on third-party libraries, it has been optimized and custom calibration processes have been added. See corresponding [documentation](https://github.com/chenxinfeng4/multiview_ball_calib).
